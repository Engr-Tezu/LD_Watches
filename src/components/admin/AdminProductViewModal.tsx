...
export default function AdminProductViewModal({
  ...
  const images = product ? getOrderedImages(product) : [];
  const waterResistant = product ? isWaterResistant(product) : false;
            ...
            >
              <div className="flex shrink-0 items-start justify-between gap-3 border-b border-ld-grey/50 px-4 py-4 sm:px-6">
                <div className="min-w-0 pr-2">
                  ...
                  <h2 id="product-view-title" className="text-lg font-bold text-white sm:text-2xl">
                    {product.name}
                  </h2>
                  ...
                  {Array.isArray(product.tags) && product.tags.length > 0 && (
                    <div className="mt-2 flex flex-wrap gap-1.5">
                      {product.tags.map((tag) => (
                        <span
                          key={tag}
                          className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] border border-ld-gold/25 bg-ld-gold/15 text-ld-gold-light"
                        >
                          {tag}
                        </span>
                      ))}
                ...
                <div className="grid grid-cols-1 gap-5 md:grid-cols-2 md:gap-6 lg:gap-8">
                  ...
                  <div className="flex flex-col">
                    ...
                    {Array.isArray(product.features) && product.features.length > 0 && (
                      <div className="mb-4 rounded-xl border border-ld-gold/15 bg-ld-dark/40 p-4 sm:p-5">
                        <h3 className="mb-3 text-xs font-semibold uppercase tracking-widest text-ld-gold-light">
                          Features
                        </h3>
                          ...
                          {product.features.map((feature, index) => {
                            const text =
                              typeof feature === "string"
                                ? feature
                                : String(
                                    (feature as { label?: string; value?: string }).label ||
                                      (feature as { value?: string }).value ||
                                      ""
                                  );
                            if (!text) return null;
                            return (
                              <li
                                key={`${text}-${index}`}
                                className="flex items-start gap-2 text-sm text-ld-light"
                              >
                                <span className="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-ld-gold" />
                                <span>{text}</span>
                              </li>
                            );
                          })}
                    ...
                    <div className="grid grid-cols-2 gap-3 text-sm">
                      <div className="glass-panel rounded-lg p-4">
                        ...
                        <p className="break-all text-white">{product.slug}</p>
                      </div>
                      <div className="glass-panel rounded-lg p-4">
                        <p className="mb-1 text-xs text-ld-silver">Images</p>
                        <p className="text-lg font-medium text-white">{product.images.length}</p>
                      </div>
                    </div>
                  </div>
                </div>
